...
#semikolon as a separator in the csv file.
...
the read_csv() and read_csv2() are tidyverse functions. by use of read_csv() to read the data into a kings object.

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
the result of running class() on the kings object created with read_csv() is "spec_tbl_df" "tbl_df" "tbl" "data.frame"

3. How many columns does the object have when created with these different functions?
then looking at the number i get 11 colums. but i have 13 after adding in 2 more columns for duration and midyear.
...
average_durationge_tibble <- kings %>%
...
select(longer_rulers, Name, duration)  
...
select(longest_rulers, Name, duration, Days)
```
